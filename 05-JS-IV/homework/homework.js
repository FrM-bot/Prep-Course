...
function multiplicarNumeroDesconocidoPorCinco(objetoMisterioso) {
  ...
  // Tu código:
  const { numeroMisterioso } = objetoMisterioso
...
function pasarUsuarioAPremium(usuarios) {
  ...
  // Tu código:
  const newUsersState = usuarios.map((usuario) => {
...
function sumarLikesDeUsuario(usuario) {
  ...
  // Tu código:
  let add = 0
  usuario.posts.forEach(({ likes }) => add = add + likes)
  return add
}
...
function agregarMetodoCalculoDescuento(producto) {
  ...
  producto.calcularPrecioDescuento = function () { 
    const finalPrice = this.precio - (this.precio * this.porcentajeDeDescuento)
    return finalPrice
  }
  return producto
}
